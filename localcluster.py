...
def printgraphforlocalcluster(x,y,edgee,start):
    damn=nx.Graph()
    ...
    for i in range(len(matrix)):
        ...
        for j in range(len(local)):
            for k in range(len(local)):
                if matrix[local[j]][local[k]] and local[j]!=local[k]:
                    gg.append([local[j],local[k],matrix[local[j]][local[k]]])
                    count+=1
        if count1==1 or count1==0:
            coeff.append(0)
        else:
            coeff.append((count/2)/((count1)*(((count1)-1)/2)))
        count=0
        count1=0
        local.clear()
        printgraphforlocalcluster(x,y,gg,i)
        gg.clear()
    avg=0
    for i in range(len(coeff)):
        avg+=coeff[i]
    avg/=len(coeff)
    print("Average coefficient of local clusters is :"+str(avg))          #to be used
def localclus(filename):
    node,x,y,edgee,startnode=inputfromfile(filename)
    edgee=minimize(edgee)
    matrix=AdjacenyMatrix(edgee,node,1)
    localcluster(matrix,node,x,y)
